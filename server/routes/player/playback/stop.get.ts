...
export default eventHandler(async (event) => {  
  ...
  try {
    ...
    logger.warn(`Error when stopping player '${targetClientIdentifier}'`, error)
    return event.respondWith(
      new Response(`Player '${targetClientIdentifier}' not available to stop playing, try again later`, { status: 404 })
    )
  }
})
