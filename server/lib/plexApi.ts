...
export const responseHeaders = (playerId: string, playerName: string, contentType?: string) => new Headers({
...
export function getPlexApiTrack(plexServer: PlexServer, track: PlexTrack, token: string): string {
  return `${plexServer.protocol}://${plexServer.host}:${plexServer.port}${track.file}?X-Plex-Token=${token}`
 ...
 */
export function metadata(track: PlexTrack): string {
  return `${track.artist} - ${track.title} (${track.album})`
}

export function extractMetadataKeyFromServerPath(url: string): string {
  const match = url.match(/server:\/\/[^/]+\/com\.plexapp\.plugins\.library(\/library\/metadata\/\d+\/children)/);
